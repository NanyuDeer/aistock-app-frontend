      ...
      <view v-else-if="alerts.length" class="alert-list">
        <view v-for="(alert, idx) in alerts" :key="idx" class="alert-item" @tap="goStockDetail(alert.symbol)">
          <view class="alert-left">
            <text class="alert-icon">{{ alertIcon(alert.type) }}</text>
...
function alertIcon(type: string): string {
  if (type.includes('涨停') || type.includes('大涨')) return '🔴'
  if (type.includes('跌停') || type.includes('大跌')) return '🟢'
  if (type.includes('放量')) return '📈'
  if (type.includes('缩量')) return '📉'
  if (type.includes('资金')) return '💰'
  return '⚡'
}
