...
export interface RhythmSummary {
  ...
  dateLabel: string
  /** 跳转 ?date= 值：恒取该行 date（详情页以 report_date/target_date 为键）；
   *  无任何数据行 → null（跳转不带参）；有数据行但均无档位 → 取首行 date（与 dateLabel「暂无档位」并存） */
  urlDate: string | null
}
...
export function formatRhythmSummary(days: RhythmCalendarDay[]): RhythmSummary | null {
  ...
  return {
    level: row.level,
    levelShort: levelShort(row.level),
    levelColor: isRhythmLevelKey(row.level) ? RHYTHM_LEVEL_COLORS[row.level] : RHYTHM_GREY,
    // 真实生产者（rhythm_engine.py:223）为「建议仓位：<阶梯文案>」（全角冒号），
    // 兼容半角冒号/空格；无前缀文本原样返回
