...
test('拒绝将 JSON 对象或数组作为 Brief 结论，避免展示原始上游载荷', () => {
  assert.equal(parseBriefingReport({
    ...EVENING_BRIEF,
    items: [{ ...EVENING_BRIEF.items[0], conclusion: '{"raw":"payload"}' }],
  }, 'evening'), null)
  assert.equal(parseBriefingReport({
    ...EVENING_BRIEF,
    items: [{ ...EVENING_BRIEF.items[0], conclusion: '["raw", "payload"]' }],
...
test('拒绝 JSON 标量和 JSON 字符串作为 Brief 结论', () => {
  for (const conclusion of ['true', 'null', '123', '"原始文本"']) {
    assert.equal(parseBriefingReport({
      ...EVENING_BRIEF,
...
test('非降级 Brief 不得声明缺失来源', () => {
  const malformed = {
    ...EVENING_BRIEF,
    degraded: false,
    missing_sources: ['iterate'],
    items: [EVENING_BRIEF.items[0], EVENING_BRIEF.items[0], EVENING_BRIEF.items[0]],
  }
  assert.equal(parseBriefingReport(malformed, 'evening'), null)
})
