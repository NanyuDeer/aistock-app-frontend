...
test('通用报告只允许公开报告类型，保留类型不会形成请求', async () => {
  const server = await createServer({
    root: process.cwd(),
    configFile: false,
    resolve: { alias: { '@': path.resolve(process.cwd(), 'src') } },
  ...
  try {
    ...
    try {
      await agentModule.agentApi.getReport('wind_leader', '2026-07-24')
      const uncheckedApi = agentModule.agentApi as { getReport: (intent: string, date: string) => unknown }
      for (const intent of ['stock', 'brief_morning', 'broadcast_evening', 'market_snapshot', 'iterate']) {
        assert.throws(() => uncheckedApi.getReport(intent, '2026-07-24'))
      }
    } finally {
      request.get = originalGet
    }

    assert.deepEqual(calls, ['/agent/report/wind_leader/2026-07-24'])
    assert.equal('getMorningBriefing' in agentModule.agentApi, false)
    assert.equal('getEveningBriefing' in agentModule.agentApi, false)
  } finally {
    await server.close()
  }
})
