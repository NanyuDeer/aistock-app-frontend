...
- 事件日历窗口放开：事件可展示至年度覆盖上限，不再截断在 5 个交易日。
- 事件区近窗平铺 + 更远折叠：近 5 个交易日（后端下发近窗末日日期）完整平铺，更远事件折叠为「更远事件（共 M 条）」可展开；后端越年返回空时近窗全部平铺兜底。

### 改进

- 日历面板适配密集日折叠占位：不渲染无内容占位行，当日溢出显示「另有 N 条」。

---

## [changer] 2026-09-21 — 节奏大师按时点自动展示 + 洞见卡显示生成时刻 + 事件日历放开 5 日限制

**开发者**: 37588

### 新增

- 三时点自动选中：进页/onShow 按上海时刻 `pickSlotByClock()` 自动选中对应 refresh_slot（<8:30→收盘基准、8:30-12:30→盘前、12:30-16:05→午间、≥16:05→收盘基准），无需手切三 tab；tab 保留，目标 slot 缺失按 SLOT_ORDER 就近回退。
- 洞见卡右上角时间改展示**节奏生成时刻**：`MM-DD · HH:MM`（日期取 targetDate、时分取所选版本 created_at 上海时区；createdAt 缺省/非法回退 slot 标签，兼容旧数据）。

### 改进

- 节奏卡事件日历标题"未来 5 交易日事件日历"→"未来事件日历"（配合 agent-py 展示窗放开为全量，事件提前展示）。
...
- 事件日历空态不再断言与事实相反的"今日无事件"，改为严格三态（有事件 / 数据源未接入 / 暂无已登记事件，互斥且按优先级判定）。
- 仓位文案的多处展示统一走同一个格式化入口，消除同屏两种格式（日历格、首页节奏条、洞见卡标题此前各写一份）。
- 日历面板事件模式的标题与空态文案与新增的交割日数据对齐，不再自称仅含"宏观事件"。

### 新增

- 情绪周期补全启动与主升两态（对齐后端五态，此前永不渲染），并补齐主升阶段的标签底色。
- 事件锚点块标题改为"下一事件"，且仅对高级事件显示"重大"强度标签。

### 重构

- 移除事件分支中从未接线的状态字段；登记废止的遗留类型槽；事件日历槽类型收紧为后端实际下发的四个字段，消除类型与线上数据不符。

### 改进

- 同步 node:test 用例基线计数。

## [feat/fear-greed-node] 2026-09-12 — 恐贪指数页面 UI 优化（历史图/洞见卡/投资建议配色）

**开发者**: Aria

### 改进
- `modules/fear-greed/pages/index.vue`：历史走势图移除冰点/沸点标记点及对应图例，替换为 20/80 分割线图例项；20/80 分割线加粗（透明度 0.15→0.4、线宽 0.6→1.0）并添加数值标注。
- `modules/fear-greed/pages/index.vue`：InsightCard 去掉「预判」行（与冰点反弹统计描述文字重复），保留「情绪洞见 + 溯源」；冰点反弹统计加卡片框（浅灰背景 + 边框）。
- `modules/fear-greed/pages/index.vue`：冰点反弹统计背景改中性透明，冰点次数/反弹概率改中性墨色，平均反弹幅度按增红减绿规则着色。
- `modules/fear-greed/pages/index.vue`：投资建议模块整体改中性色（仓位条、配置方向标签、操作要点圆点、弹窗标题），仅保留分档徽章（冰点/寒冷/常温/温热/沸点）原始配色。
...
### 改进
- `modules/market/pages/leaders.vue`：长线/短线两档榜单排序统一为**先按上榜次数降序**（长线=近120日 `frequency`、短线=`freq20`）**再按该档持续时间降序**（长线 `long_term_days` / 短线 `short_term_days`）；此前长线按持续天数优先、短线次级按热度，与"先上榜次数"诉求相反
...
## [changer] 2026-09-03 — App 更新「已是最新」假阳真机排障与修复 + 0.1.3 发布

**开发者**: NanyuDeer

### 修复
- **正式包读版本号 = 0 假阳根因（多轮排障收敛）**：`plus.android` 反射读 `getPackageInfo` 在正式包返回 null（flags=0/matchAll/getAttribute/longVersionCode 均不可靠），且 `importClass` 取 `PACKAGE_MATCH_ALL` 返回 null 被误判成功 → **弃用原生反射**，改官方 `plus.runtime.version` 读 versionName + `compareVersion` 逐段字符串比较（`getCurrentVersionName`/`parseVersion`/`compareVersion`）；`checkAppUpdate` 由 versionCode 数值比较改为 versionName 比较，`isNeverUpdate` 存储 key 仍用 `info.versionCode`
- **version.json 缓存命中旧版本号**：`fetchLatestVersion` 请求追加 `?t=${Date.now()}` cache-busting（带 Etag/Last-Modified 的 version.json 会命中 HTTP/App 缓存旧 versionCode 导致「已最新」）
- 真机验证通过后移除全部诊断代码：`updateDiag` 快照、profile「版本检测诊断」弹窗、`[appUpdate-diag]` console 日志

### 发布
- `src/manifest.json`：versionName 0.1.2 → 0.1.3、versionCode 102 → 103（供正式打包）；配套 `aistock-frontend public/download/version.json` 0.1.3/103（fileSize 待打包后修正）
- 语义说明：版本比较改用 versionName 字符串；线上 version.json 的 versionName 需与 App 打包 versionName 保持一致

## [changer] 2026-09-02 — 板块四环与条件化预判块（sector-loop + ConditionalForecastBlock 全粒度共用）

**开发者**: NanyuDeer

### 新增
- `src/modules/market/pages/sector-loop.vue` 板块四环页（SubPageCard 白卡容器；大盘归因链「上证 涨/跌 → 主因板块（±x%）」；行=板块名+来源 tag(风口/大盘主因/风口·主因)+溯源短句+当日行情+预判概要；空态「收盘后数据更全」）；`pages.json` 注册；leaders.vue 加「板块预判 · 今日 ›」入口；traceability.vue MarketInsightCard 后加「主因板块 · 板块研判」区块
- `src/shared/components/ConditionalForecastBlock.vue`（新，同步组件库）：从 InsightCard structured 抽取的通用条件化预判块——期段 Tab + 每期基准方向/置信/剩余时长 + 互斥分支 + met 触发点亮/置灰 + 验证 pill + 验证锚点 chip（anchor.threshold/metric 透传）；凡有条件化预判的粒度（大盘/板块/个股）一律渲染
- `src/shared/components/SectorInsightCard.vue`（新）：板块洞见卡 wrapper（candidate 命中渲染 InsightCard，null 严格占位 D4）
- `src/shared/api/modules/agent.ts`：`agentApi.getSectorInsight(date)` + SectorInsight 类型族（direction/confidence 窄联合）；InsightCard 新增 `structured` prop + `tag-text` 覆盖（文本形态零破坏兼容）
- `src/shared/utils/sectorInsight.ts`（新）：`sectorPredictionToStructured`（单一映射源）/`findSectorCandidate`（code/.TI/name）/`todayDateStr`
- `src/shared/utils/conditionalForecast.ts`（新）：`expandConditionalBranches`——scenario 内嵌“；若X则Y”情形拆为独立条件条目（方向/锚点随主卡置空）
- sector-detail.vue AI 分析卡洞见化回退 + 板块洞见卡嵌入（最近交易日匹配 candidate，位于 AI 分析后、K线前）

### 改进
- 条件化预判条件行双模式：`conditionDisplay='tags'`（默认，有 keywords 显示标签流、无则长句兜底）/ `'sentence'`（强制原文，大盘详细报告用）；`keywords?: string[]` 透传（板块洞见卡/四环列表标签化）
- 板块四环列表主因置顶分组（review_primary/both 红强调 + “大盘溯源·主因板块”标题）+ 行内预判详情区复用共享预判块
- 日期回看由横向胶囊 →「‹ 日期 › 步进 + 浮层列表（20 交易日一键回今天）」→ 下拉面板，最终默认显示前前交易日（今日/昨日数据常未完成）
- 大盘条件化预判按期段分组分支卡（MarketTracePrediction 嵌入 ConditionalForecastBlock）；对冲分支方向取主条件反向（主 bullish→bearish）
- 弱溯源还原原文为溯源横幅样式展示；无明确触发时精简为「当日无单一明确触发事件（溯源证据不足）」

### 修复
- H5 dev 板块四环「加载失败」：`vite.config.ts` 补 `/api/agent/sector-insight` 代理 → Node app-api（此前落入 Python 兜底 404）

### 文档
- modules/market/AGENTS.md、modules/analytics/AGENTS.md、根 AGENTS.md §7 组件表同步（InsightCard 升级 + SectorInsightCard 新增）

## [changer] 2026-08-31 — 账号安全页补全手机号关联

**开发者**: NanyuDeer

### 新增/改进
- `src/modules/user/pages/account-security.vue`：当前绑定新增「手机号」状态行（脱敏 138\*\*\*\*0000）；新增「绑定手机号」入口；绑定表单三模式（phone/email/wechat）——绑定手机号/邮箱输入新身份，绑定微信改用**当前账户已绑定身份证明归属**（邮箱优先、手机号次之，只读展示脱敏身份）；说明文案更新为三者互绑 + 冲突自动合并
...
- `RhythmCard.spec.ts`：P1 锚点渲染源码断言（next\_event\_anchor / v-if / 下一重大事件 / rc-anchor）

### 文档

- `src/modules/rhythm/AGENTS.md`：渲染契约表追加 next\_event\_anchor 字段行
...
- `PredictionVerification.vue`：新增条件化预判验证渲染（按 c{i}），融合 A1 early\_exit 失效信号，horizon 验证保持兼容
...
- 首页「今日专属」报卡补充午间报：`src/modules/home/components/MorningContent.vue` 按时间自动切换晨报/午间报/晚报（12:00–15:30 显示午间报，摘要+短关键词标签与晨/晚报样式统一）；`src/shared/utils/useBriefingCard.ts` 扩展支持 `midday` 类型（走 `/agent/report/midday/:date` 的 display\_report 结构，晨/晚报仍走 `/agent/brief/:type/:date`）；`briefingTypeAtShanghaiTime` 时段划分：15:30+ 晚报 / 12:00–15:30 午间报 / 早间晨报
...
- `src/modules/favorites/pages/insight.vue` / `insight-detail.vue` / `monitor.vue`：归因状态与 primary\_cause 展示适配
...
- `src/shared/utils/useAppUpdate.ts`：`checkAppUpdate({ manual })` 版本检查——非 Android App 环境返回 not\_supported；启动自动检查 24h 节流（storage key `app_update_last_check`）；有新版本弹窗 → `uni.downloadFile` 下载 → `plus.runtime.install` 安装；本机 versionCode 经 `plus.android` 原生 PackageManager 读取

- `src/App.vue`：APP-PLUS 端启动后 3s 静默执行 `checkAppUpdate()`（自动更新检查）

- `src/modules/user/pages/profile.vue`：菜单新增「版本更新」项 → `checkAppUpdate({ manual: true })` 手动检查（latest/not\_supported/error 分别 toast）
...
- `src/modules/market/pages/leaders.vue`：
  ...
  - 短线风口榜排序由「短线持续天数 short\_term\_days → freq20」改为**上榜次数（近10日 freq20）→ 热度（short\_heat）降序**（长线榜保持 long\_term\_days → frequency 不变），与后端 `applyDualRankings` 短线口径统一；修复短线档原按 AI 天数排序与"上榜次数-热度"预期不符的问题
...
- 重新云打包 APK（含 Record 模块 + RECORD\_AUDIO）
...
- `src/modules/favorites/pages/monitor.vue`：异动监控数据源从已停用的 stock\_trace 切换到自选股洞察 API，卡片展示主因 / 置信度（高置信/待验证）/ 日期，点击进入洞察详情页；移除"全部/大涨/大跌"筛选分栏，所有异动事件直接平铺展示；"立即检测"改为刷新列表（洞察由后端 cron 周期采集）
...
## \[changer] 2026-08-06 — ChatAgent 会话用量徽标 + 单轮用量进气泡 + 气泡消失修复 + HTTP 降级 token\_usage + WS 端口对齐 8080
...
- HTTP 降级路径 token\_usage 透出（`useChatStream.ts`）：降级分支 `appendMessage` 透出 `tokenUsage: result.token_usage`（此前恒 undefined）
...
- `src/shared/utils/useChatStream.ts`：socket 模块级单例（页面 onUnmounted 不再 disconnect，后台生成继续）；`hasPendingRun()` / `resume()` / `isConnected()`（onShow 回页自动续跑，resume\_status none 自动重发最后一条 user 消息兜底）；`stop()` / `retry()` / `hasStoppedRun()`；handleWsMessage 新增 resume\_status / stop\_status / cancelled 分支（本地兜底落消息 + 去重）

- 遗留补丁：stop\_status 置 doneReceived（防后端 stop 后迟发残留 text/done）；resume 轮断连结算（模块级 resumeInFlight 标记——断连只结算 streaming、不落错误消息，保留 pending 供 onShow 再 resume）
...
- `src/shared/api/modules/agent.ts`：`createAgentWebSocket` URL 追加 `?token=`（app-api 桥接握手鉴权）；`sendMessage` 删除 user\_id 字段（服务端注入）；删除未使用的 useUserStore import

- `src/shared/utils/useChatStream.ts`：`send` 删除 user\_id 字段；新增断连结算（4401/连接断开时结算挂起 send，走错误提示，不挂起 streaming）

### 文档

- AGENTS.md / src/modules/chat/AGENTS.md：user\_id 服务端注入约束更新

> 发版约束：须在网关 WS 路由已指向 app-api 之后发布；此前前端 WS 仍直连 agent-py，删除 user\_id 会导致登录态 WS 不落库不计费。
...
- `marketTraceReview.spec.ts` 新增 5 用例（prediction 提取三态 + evolution\_steps 映射 + 旧记录兼容），15 通过
...
- `src/modules/analytics/pages/prediction-history.vue`：预测验证列表页（命中率统计栏 + 全部/进行中/已结束筛选 + 预测卡片含 prediction\_status 与三档进度）
