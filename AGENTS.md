...
| 模块文件 | 说明 | 后端路径 |
|---------|------|---------|
| `agent.ts` | Agent 反代（SSE 流式对话、分析报告查询、音频服务；P3-fix 新增 `ReasoningStep` 类型 + `ChatMessage.reasoningSteps`，WS reasoning 协议契约；P9 会话管理：`ChatSessionMeta` 类型 + `listChatSessions`/`upsertChatSession`/`deleteChatSession`；**P0 身份鉴权：`createAgentWebSocket` URL 带 `?token=`（app-api 桥接验签）、`sendMessage` 不再携带 `user_id`（服务端注入）**；**批次 2：`getChatAnalysisReport(reportId)` 深度分析报告详情查询（`/api/agent/report/chat/:reportId`，显式解包返回 `ChatAnalysisReport | null`）+ `ChatAnalysisReport` 类型（`content.display_report` 双层结构）**；**午间报（2026-08-24）：`MiddayReportRecord` 类型（`content.display_report` + `content.audio_path` 可选，方案 A；`content.display_report.sections[].opportunities` 可选，schema 2.1）**；**节奏大师（2026-08-30）：`RhythmMasterReport`/`RhythmSlot` 系列类型 + `getRhythmMaster(date)`（`/api/agent/rhythm-master/:date`，三时点 refresh_slot 版本）**；2026-09-18（R17）：`SectorInsightCandidate.source` 类型抽为导出 `SectorInsightSource`（`wind_leader｜review_primary｜both｜chain_only`），新增值 `chain_only` = **前端合成**候选（仅当日归因链上有、sector-insight 候选里没有的板块，`quote/trace/prediction` 恒 null；见 `sectorInsight.buildPrimarySectorCandidates`）**） | `/api/agent/*`；P9 会话 `/api/chat/sessions` |
| `attributionChain.ts` | 大盘归因链（P1 chain-attribution，2026-09-04 自 modules/market/api 提升至共享层）：`fetchAttributionChain(date)` → `GET /api/agent/attribution-chain/:date`，返回 `{date, chain\|null}`；无链/请求失败返回 null 不抛出；类型 `AttributionChain`/`AttributionChainChild` 同文件导出。**2026-09-17 弱依据加性扩展（R16，全部可选）**：`root.evidence_weak?: boolean`（true=当日大盘未确认主因）+ `root.attribution_status?: string`（如 `hypothesis`）；`child.extraction?: { source?: string; weak?: boolean }`（`candidate_claim`=候选链 claim 兜底 / `snapshot`=快照异动兜底）；`child.ts_code?/sector_std?`（角色徽匹配用，可能缺失）；老数据/正常日缺省 → 前端零变化 | `/api/agent/attribution-chain/*` |
| `auth.ts` | 认证（登录、用户信息）。2026-09-26 新增密码登录/注册：`passwordLogin(account, password)` → `POST /api/auth/password/login`（仅**账号维度**节流，同账号 15 分钟内失败达 10 次才 429，仅提示不降级）、`register(account, password, code)` → `POST /api/auth/register`（注册即登录；存量未设密码账号可**补设密码**；已设密码返回 409）。`UserInfo` 含 `hasPassword`（`/users/me` 权威字段，`fetchUserInfo()` 同步） | `/api/auth/*` |
...
| `InsightCard.vue` | [组件库] | 洞见卡（2026-09-02 升级：新增 `structured` prop 渲染条件化预判 + `tag-text` 标签覆盖；文本形态 trace/forecast 零破坏兼容）。structured 预判部分由内部通用块 ConditionalForecastBlock 承载（2026-09-02 抽取）。文本用法：title/trace/forecast/time/theme/showMeta/confidence，emit click；市场洞见页 MarketInsightCard 复用。**2026-09-17（P3' Task 4.2）**：`traceStructured` 加性扩展 `events?: InsightTraceEvent[]`（`{headline, ref?, source?}`，链上事件节点）——渲染在溯源行下方（空数组/缺省不渲染该区），胶囊点击 emit `eventSelect(ev)` 由调用方跳原文。**2026-09-17（弱依据提示，R16）**：`InsightTraceStructured` 再加性扩展 `weak?: boolean`（链级：`root.evidence_weak=true` → 摘要行旁中性灰小标「归因较弱」）与 `weakText?: string`（板块级：`child.extraction.weak=true` → 角色徽驱动行旁中性灰小标，文案由 `sectorInsight.extractionWeakLabel` 单点产出：「依据较弱」/「无归因依据」）；两者均**可选渲染**（字段缺失/正常日零标记、零变化），刻意不用告警色与涨跌色。**2026-09-18（R17）**：新增可选 `titleTag?: string` → 标题**上方**中性描边小标（`.as-insight-card__name-tag`，底色/文字走 `--ins-fc-bg`/`--ins-card-tx` 主题变量），供"标题是溯源主句、看不出板块名"的板块卡使用；缺省不渲染（其他调用方零变化） |
| `ConditionalForecastBlock.vue` | [组件库] | 通用条件化预判块（2026-09-02 抽取，大盘/板块/个股凡有条件化预判一律复用，与板块洞见卡同款 UI）：props { structured: { horizons/conditions/dueLabel/verification } | null }——期段 Tab（短/中/长）+ 每期基准方向/置信/剩余 + 互斥分支（若 条件 → 方向 pill → scenario 幅度置灰）+ met 触发点亮/置灰 + 验证 pill + anchor threshold/metric chip。InsightCard structured 与 analytics MarketTracePrediction 共用。**2026-09-17 未触发折叠态（同日判定修正）**：折叠/过滤改按「当期有无已成立分支（`met===true`）」判定，**不再依赖 `resolvedDisplayMode` 降级**（后端只写 `condition_met=true`，未触发档无任何布尔 met，旧口径会降级 `full` 使折叠态不可达）；当期无已成立分支且该档有条件时，分支区不铺开、收为一行入口「查看条件化预判 ▾」/「收起条件化预判 ▴」（本地展开，切期段归零；展开后铺开该档全部分支），基准行照常；此形态下原空态文案不再显示（仅 `sentence` 形态保留）。折叠态若 `verification === 'miss'` → 入口旁中性灰「未命中」且头部同义 pill「验证未中」被抑制；有已成立分支时只渲染已成立分支 + 分支区末尾纯标注「另有 N 条条件未成立」。**2026-09-17 复审收口（同日第二修）**：折叠/过滤/隐藏标注/「未命中」标签**一律以 `displayMode === 'conclusion'` 收口**——未传或 `full` 的调用方（节奏大师洞见卡等）恒全量分支直显、无折叠入口（否则其卡内默认只剩一行入口 = 核心分支不可见）；空态文案还原改造前口径（`sentence` → 「该期暂无细分情景」、tags 结论态 → 「条件未成立 · 暂无已验证结论」）；三态渲染护栏见 `ConditionalForecastBlock.mount.spec.ts`（vitest 挂载 5 条） |
| `SectorInsightCard.vue` | App 专属 wrapper | 板块洞见卡（2026-09-02，板块四环）：props { candidate: SectorInsightCandidate\|null; loading?; date?; marketLink?; sectorName?; displayMode?; **traceOnly?**; traceDetail? }——candidate 命中渲染 InsightCard（板块洞见 + 条件化预判），null 渲染"暂无板块研判"严格占位；数据由父页从 agentApi.getSectorInsight 拉取。**2026-09-17 traceOnly（spec §2.1 两轨分离）**：为 true 时只渲染溯源侧——不传 `structured`（不渲染 CFB 预判子卡）、标题不回退预判综述（取 `candidate.trace.summary` → 角色兜底标题），并保留「依据详情 ▾」；`marketLink.events` 透传为 InsightCard 溯源子卡事件胶囊（点击经 `@event-select` → H5 window.open / App webview 承载页跳原文）；复用点 sector-detail（风口详情）、sector-loop 与 traceability（主因板块，`trace-only`）。**2026-09-17 弱依据透传（R16）**：`marketLink.chainWeak` / `marketLink.extraction` → InsightCard `traceStructured.weak` / `weakText`（链级「归因较弱」+ 板块级「依据较弱」/「无归因依据」）；匹配侧调用方传 `{ code: sector.ts_code }` 走 R14 优先级。**2026-09-18（R17）**：`traceOnly` 下标题上方渲染**板块名标签**（`sectorName` 优先、回退 `candidate.name`，透传 InsightCard `titleTag`）——主因卡标题是溯源主句，单看卡看不出板块名；同轮 `inChain` 扩展为「有 relation **或** traceOnly 下有链上驱动句」（链 only 合成卡无候选 trace，relation=unknown 时仍需出卡）。**2026-09-19（组长裁定，修「所有板块详情的溯源都是同一句大盘结论」）**：① **未入链一律不渲染「大盘联动」结构化溯源块**——`traceStructured` 判定改为「**有 `relation` 或该板块 `driver` 非空**」（原判据只看 `summary`，而 `buildMarketLink` 在未命中链节点时仍填 `chain.root.summary`，导致链只覆盖 2 个板块时其余全部显示同一句大盘结论；未入链 → `null` 回退该板块自己的溯源文本，无则整块不渲染）；② **板块粒度不再显示「待验证」pill**——`sectorPredictionToStructured` 只保留 `hit/miss`，`pending` 归一 `null`（CFB 头部 pill 消失；折叠态由 `met` 驱动，不受影响）。 |
| `AttributionChainView.vue` | App 专属 wrapper | 大盘归因链视图（2026-09-03 P1 chain-attribution，2026-09-04 自 modules/market/components 提升，消除 analytics→market 跨模块依赖）：**2026-09-17 受控化**——props { date: string; chain?: AttributionChain \| null; loading?: boolean; mock?: boolean }，链数据由页面 `fetchAttributionChain` 拉取后 `:chain` 传入（页面需同一份链做卡排序/marketLink 匹配）；无链/失败由组件内空态承接（`loading` / `chain=null`）；`mock=true` 渲染内置演示数据（仅演示用）；大盘根卡（一句话主因 + index_pct 涨跌）→ 主驱动板块分支（relation 中性徽：自驱动/跟随大盘/关系未知 + 板块一句话溯源驱动卡 + **链上事件胶囊 EventRefChip**，URL 可跳原文：H5 window.open / App+小程序 webview 承载页）；复用点 traceability（大盘溯源页）。**2026-09-17 板块行弱依据标记（R16）**：`child.extraction.weak=true` → 关系徽旁中性灰小标（`extractionWeakLabel`：「依据较弱」/`snapshot` 的「无归因依据」）；缺省/正常日不渲染。**2026-09-18（R17）**：children 渲染前过滤「未确认驱动原因」节点（`sectorInsight.isUnconfirmedAttribution`：`trace_summary` 去空白为空或命中中性未确认表述；events 多是行情综述≠驱动原因，故只看摘要）——与市场洞见页主因卡同一口径；大盘根不受影响。**2026-09-18 晚（组长两项裁定）**：① **链上只渲染「中台」（`source === 'warehouse'`）事件胶囊**——`search`（板块定向检索补漏，多为行情综述/研报观点/栏目碎片）在链上隐藏（`warehouseEvents(c)` 过滤；全被滤掉 → 事件区不渲染、不占位，旧链无 `events` 行为不变）；② **撤掉每分支「看该板块预判 →」入口**（`.acv-forecast` 模板块 + `defineEmits(['select-sector'])` 一并删除，预判改由板块详情页/风口页进入）——分支至此只留溯源侧：关系徽 + 板块名 + 驱动句 + 事件胶囊 + 「溯源过程 ▾」。同轮已有 `sectorStages` prop + 分支配「溯源过程 ▾」展开（3 段 触发/传导/结果，键三级降级 `ts_code → sector_std → 原始名`，展开态按分支独立；无数据不出入口）。**2026-09-19（组长裁定，方案 E）**：① **用户可见名改为「今日驱动板块」**（内容主体是「哪些板块驱动大盘」，与板块更相关；组件名 `AttributionChainView` / API `attribution-chain` **不改**，仅改标题文案与文档表述）；② **视觉改向洞见卡看齐**——卡内改为「加粗彩色 key（`大盘` / 板块名）+ 正文（= 归因结论 `trace_summary`）+ 渐变分隔线」的洞见卡行语言（分隔线与 `InsightCard` 同款 `linear-gradient(90deg, $primary-100, rgba($primary-100,0))`），关系徽 `.acv-badge` 更名 `.acv-rel`，展开入口文案「溯源过程」→ 洞见卡同款「依据详情」；③ **去掉重复底色块**：原「大盘根卡 `$bg-soft` 浅底块 + 每分支 `$primary-50` 浅蓝驱动句块」全部改为无色块正文（同重量色块导致看不出哪个板块是主因）；④ **大盘行与板块分支同构**（key 行 + 结论另起一行），且 key「大盘」比板块名更显眼——**同取品牌蓝**，靠「字号（`$font-size-md` 28rpx vs `$font-size-sm` 24rpx）+ 字重（700 vs 600）」区分；同行指数涨跌**右对齐**（`justify-content: space-between`）（首版曾改用 `$ink` 墨色，组长裁定颜色仍用蓝色）。**纯样式/文案变更，逻辑与判别口径零改动**（R16 弱标记 / R17 未确认过滤 / 只展示中台事件 / `sectorStages` 3 段展开全部保留）；展开入口文案已与 `sector-loop.vue` 四环行统一为「依据详情」 |
| `EventRefChip.vue` | [组件库] | 链上事件胶囊（2026-09-17 P3' Task 4.2，spec §3.2-4/§7）：props { headline: string; source: 'warehouse' \| 'search'; eventRef?: string }——来源标记（中台/检索）+ 事件摘要单行省略；`eventRef` 为 http(s) URL 时点击 emit `select(url)`（跳转由调用方按平台惯例执行，组件纯 UI 不导航），非 URL（`event:<id>` / `search:<query>\|<title>`）不可点纯文本展示；接入点 AttributionChainView（每分支 ≤3 条；**2026-09-18 晚起链上只渲染 `warehouse` 来源**，`search` 在链上隐藏）与 InsightCard 溯源子卡（`traceStructured.events`） |
...
| `useChatStream` | 对话流（WS 为主，HTTP 降级；`send(content, { forceDeep })`；DONE 写 `execSteps`/`lastDeepReport`，P3；订阅 reasoning 聚合 `reasoningSteps` + `_testHandleWsMessage` 测试钩子，P3-fix；return `streamingReasoning` 流式实时思考链，P3-fix-2；**P0：send 不再携带 user_id（服务端注入）；连接断开/4401 时结算挂起 send（不卡死 streaming）**；**Phase 4-2：`confirm_request` 终态处理（doneReceived 置位 + pendingConfirm ref + 结算 send promise）+ `sendConfirmResponse(request_id, choice)` 发送成功后 re-arm（doneReceived=false/streaming=true/清 progressSteps/streamingText/currentRunReasoning/currentRunEvents）**；**问题 20 R3：WS 发送后 idle 静默段超时兜底**（`_STALL_TIMEOUT_MS=1800_000` 30min 校准期 P95 + `_STALL_CHECK_INTERVAL_MS=10_000`；超时落「生成超时，请稍后重试」+ 复位 streaming + 发 `{type:"stop"}` 联动后端 finalizing 护栏 + 结算 send promise）） |
