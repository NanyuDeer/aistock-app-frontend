...
| `ConditionalForecastBlock.vue` | [组件库] | 通用条件化预判块（2026-09-02 抽取，大盘/板块/个股凡有条件化预判一律复用，与板块洞见卡同款 UI）：props { structured: { horizons/conditions/dueLabel/verification } | null }——期段 Tab（短/中/长）+ 每期基准方向/置信/剩余 + 互斥分支（若 条件 → 方向 pill → scenario 幅度置灰）+ met 触发点亮/置灰 + 验证 pill + anchor threshold/metric chip。InsightCard structured 与 analytics MarketTracePrediction 共用。**2026-09-17 未触发折叠态**：`conclusion` 实际生效且当期无 `met===true` 分支时，分支区不铺开、收为一行入口「查看条件化预判 ▾」/「收起条件化预判 ▴」（本地展开，切期段归零；展开后铺开该档全部分支），基准行照常；此形态下原空态文案不再显示（仅 `sentence` 形态保留）。折叠态若 `verification === 'miss'` → 入口旁中性灰「未命中」；有已成立分支时分支区末尾纯标注「另有 N 条条件未成立」 |
